...
export function HexGlobe() {
  ...
  useEffect(() => {
    ...
    for (const { center, verts2d } of cells) {
      ...
      const cmat = new THREE.MeshLambertMaterial({ color: '#698cc9', emissive: '#ffffff', emissiveIntensity: 0.2 })
      cloudMats.push(cmat)
    ...
    seaMat.onBeforeCompile = (shader) => {
      ...
      shader.vertexShader = shader.vertexShader.replace(
        ...
        `#include <begin_vertex>
        float wave = sin(position.x * 6.0 + uTime * 1.2) * cos(position.z * 6.0 + uTime * 0.9)
                   + sin(position.y * 8.0 + uTime * 0.7) * 0.5;
        vWave = wave;
        transformed += normalize(position) * wave * 0.017;`
      )
      ...
      shader.fragmentShader = shader.fragmentShader.replace(
        ...
        `#include <color_fragment>
        float t = clamp(vWave * 0.8 + 0.1, 0.0, 1.0);
